...
class Search {
  ...
  getResults() {
    ...
    const urls = [`${universityData.root_url}/wp-json/wp/v2/posts?search=${this.searchField.value}`, `${universityData.root_url}/wp-json/wp/v2/pages?search=${this.searchField.value}`, `${universityData.root_url}/wp-json/wp/v2/program?search=${this.searchField.value}`];

    // Mapea las URLs a promesas fetch y las ejecuta en paralelo con Promise.all
    Promise.all(urls.map(url => fetch(url))).then(respuestas => {
